#! /usr/bin/awk -f
#
# extract-del2rho.awk - Extract bond critical point properties from
# the 'slightly verbose' output of extreme.
#
#-----------------------------------------------------------------------
# CopyRight (c): Victor Lua~na, Dec. 20, 1999
...
BEGIN {
   ...
   radius[type_nnm] = 0.04
   radius[type_nuc] = 0.04
...
#      printf "%12.6f %12.6f %12.6f ", hess1[i], hess2[i], hess3[i]
   ...
   poincare = ncp[type_nuc] + ncp[type_nnm] - ncp[type_sd1] + ncp[type_sd2] - ncp[type_min]
   printf "Poincare-Hopf sum (n-b+r-c): %6d\n", poincare
...
#   print "Atomos y puntos (3,+3) en del2rho" > file
   ...
   print "set camangle 65 15 35"                                > file
#   print "set background background {color rgb <0.6,0.9,0.8>}"  > file
   print "set background background {color rgb <1.0,1.0,1.0>}"  > file
   print "set use_planes .false."                               > file
   print "set zoom 2"                                           > file
   print "set ball_texture Dull"                                > file
   print "set stick_texture Dull"                               > file
   print "molecule"                                             > file
   print "   crystal"                                           > file
   print "      spg p 1"                                        > file
   ...
   print "   symmetry"                                          > file
   print "      molorient"                                      > file
   print "   endsymmetry"                                       > file
   for (i in atomos) {
      iZ = atomos[i]
#      printf "   BALL %-4s radius %7.3f CPK\n", i, radius[type_atom] > file
      printf "   BALL %-4s radius %7.3f JMOL\n", i, radius[type_atom] > file
...
#                    i, radius[type_atom], CPK[col[iZ]]  > file
   ...
   print "run convert grafo.tga png:grafo.png"                  > file
