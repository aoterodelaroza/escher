...
#
# extract-xyz-g09.awk - Analysis of the gaussian g09 output
# Get xyz files for all the geometries contained in the output.
#
#
#-----------------------------------------------------------------------
...
/Input orientation:/ { target = FNR + 5; steps++; doxyz=1 }
FNR==target, /^ ------*$/ {
   if (NF>5) {
      nat = $1
      atZZ[nat] = $2
      atname[nat] = atomicname[$2]
      atxc[nat] = $4
      atyc[nat] = $5
      atzc[nat] = $6
      }
