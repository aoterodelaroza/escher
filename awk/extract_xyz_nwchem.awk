...
#
# extract-xyz-nwchem.awk - Analysis of the NWChem output
...
/Output coordinates in angstroms/ { target = FNR + 4; steps++; doxyz=1 }
FNR==target, /^ *$/ {
   if (NF>0) {
      nat = $1
      atname[nat] = $2
   ...
   else if (NF==0) { target = -100 }
   }

doxyz==1 && FNR>target+5 && /^ *$/ {
