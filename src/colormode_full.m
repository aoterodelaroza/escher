...
function rgb = colormode_full(u,f,scale=[])
% function rgb = colormode_full(u,f)
%
% colormode_full - color function generator for rep_surface. Given an 
...
%   scale = [fmin fmax] is given. The color scheme is rainbow-like.
%
% Required input variables:
% u: array (n,2) of points on the surface.
% f: z = f(u) scalar function.
% scale: [fmin fmax] values for the scale.
%
...
%
  ...
  for i = 1:size(u,1)
    ...
    rgb(i,1:3) = 2 ./ (1 + exp([(ff-1)^2, 3*ff*ff, (ff+1)^2]));
  endfor
