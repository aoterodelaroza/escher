...
%
  ...
  for i = 1:rep.nball
    ...
    found = 0;
    for j = 1:nrgb
      if (all(abs(rep.ball{i}.rgb - rgb(j,:)) < 1e-5) && strcmp(tex{j},rep.texlib{rep.ball{i}.tex}))
        found = 1;
        rep.ball{i}.irgb = j;
      endif
    endfor
  ...
  for i = 1:rep.nstick
    ...
    found = 0;
    for j = 1:nrgb
      if (all(abs(rep.stick{i}.rgb - rgb(j,:)) < 1e-5) && strcmp(tex{j},rep.texlib{rep.stick{i}.tex}))
        found = 1;
        rep.stick{i}.irgb = j;
      endif
    endfor
